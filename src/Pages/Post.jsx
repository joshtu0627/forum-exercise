...
import { Grid, Item, Image, Icon, Container, Header, Segment, Comment, Form, Message } from 'semantic-ui-react'
...
export default function Post() {
    ...
    React.useEffect(() => {
        firebase.firestore().collection('posts').doc(postId).collection('comments').orderBy('createdAt', 'desc').onSnapshot((snapshot) => {
    ...
    function toggle(isActive, field){
        ...
        firebase.firestore().collection('posts').doc(postId).update({
            ...
            [field]: isActive ? firebase.firestore.FieldValue.arrayRemove(userId) : firebase.firestore.FieldValue.arrayUnion(userId)
        })
        // if(isActive){
        //     firebase.firestore().collection('posts').doc(postId).update({
        //         // 在該文章的被誰收藏的欄位中移除目前使用者的id
  ...
  return (
    <Container>
        ...
        <Image src={post.imageUrl || 'https://react.semantic-ui.com/images/wireframe/image.png'} fluid/>
        ...
        <Segment basic vertical>
            ...
            <Icon name={isCollected ? 'bookmark' : 'bookmark outline'}  color={isCollected ? 'blue' : 'grey'} link onClick={()=>{toggle(isCollected, 'collectedBy')}} />
        </Segment>
        <Comment.Group>
            <Form reply>
            ...
            {comments.map((comment) => {
                return (
                    <Comment>
                        <Comment.Avatar src={comment.author.photoURL || 'https://react.semantic-ui.com/images/avatar/small/matt.jpg'}/>
                        <Comment.Content>
                            <Comment.Author as="span">{comment.author.displayName || '匿名使用者'}</Comment.Author>
                            <Comment.Metadata>{comment.createdAt && comment.createdAt.toDate().toLocaleString()}</Comment.Metadata>
